 ...
 *   이벤트 리스너는 컴포넌트에 연결되어 있어야 독장을 함.
 *   즉, 화면에 구성된 컴포넌트들은 이벤트 리스너를 하나씩 가지고 있음.
 * - 컴포넌트와 리스너를 연결시키는 메서드 : addXXXXListener() ==> XXXX는 리스너 인터페이스
 * - 이벤트 리스너 작성 과정.
 ...
 *   2) 버튼 클릭은 운영체제의 마우스 드라이버를 거쳐 자바 가상기계(JVM)에 전달이 됨.
 *   3) 자바가상기계는 이벤트 분배 스레드에게 마우스 클릭에 관한 정보를 전달함.
 *   4) 이벤트 분배 스레드는 이벤트(ActionEvent) 객체를 생성함. 이벤트 객체는
 *      이벤트에 관련된 여러 정보를 담고 있는 객체임. 이벤트 객체 내에 저장되는 정보
 *      중에 특별히 이벤트를 발생시킨 컴포넌트를 이벤트 소스라고 부름. 여기서 이벤트
 *      소스는 버튼에 글자가 새겨진 JButton 컴포넌트임.
 *   5) 이벤트 분배 스레드는 JButton에 연결된 버튼 컴포넌트와 관련된 이벤트 리스너를
 *      찾아서 호출을 함.
 ...
 *             마우스 버튼의 종류, 눌려진 키)를 담는 객체로서, 이벤트에 따라
 *             서로 다른 정보가 저장됨.
 * - 이벤트 리스너(Event Listener) : -이벤트를 처리하는 코드로서 컴포넌트에
 *                                등록이 되어야 작동이 가능함.
 *                              - 이벤트 소스와 이벤트 핸들러를 연결.
 * - 이벤트 분배 스레드 : 이벤트 기반 프로그래밍의 핵심 요소로서 무한 루프를 실행하는
 *                 스레드임. 자바 가상 기계로부터 이벤트 발생의 통지를 받아서,
 *                 이벤트 소스와 이벤트 종류를 결정하고 이에 따라 적절한 이벤트
 *                 객체를 생성하고 이벤트 리스너를 찾아 호출함.
 */

/*
 * 이벤트 처리 방법 - 첫번째
 * - 이벤트와 이벤트 리스너(ActionListener) 선택
 * - 이벤트 리스너(인터페이스)를 상속을 받음. 
 * - 이벤트에 맞게 추상메서드 재정의.
 */
