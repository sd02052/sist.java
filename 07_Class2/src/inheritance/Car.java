 ...
 * - 실세계에서 상속은 상위 개체의 속성이 하위 개체에 물려져서
 *   하위 개체가 상위 개체의 속성을 모두 가지는 관계임.
 * - 자바의 상속은 자식클래스가 부모클래스의 속성을 물려받고
 *   기능을 추가하여 확장(extends)하는 개념임.
 * - 상속은 슈퍼클래스의 필드와 메서드를 물려받아서 코드를
 ...
 * - 자식과 부모의 관계로 형성되어 있음.
 *   ==> 부모 클래스 : Super, Parent 클래스라고 하기도 함.
 *   ==> 자식 클래스 : Sub, Child 클래스라고 하기도 함.
 *   ==> 자식은 부모의 멤버보다 같거나 많다.
