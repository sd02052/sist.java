 ...
 * 	있다고 볼 수 있음.
 * - 객체지향 프로그래밍(OOP : Object Oriented Programming)은 만들고자
 * 	하는 완성품인 객체를 모델링하고, 집합관계이 있는 부품 객체와 사용관계에 있는 객체를 
 * 	하나씩 설계한 후조립하는 방식으로 프로그램을 개발하는 기법임.
 */

/*
 * 클래스(Class)?
 * - 현실에서 객체는 갑자기 만들어지는 것이 아니라, 설계도를 바탕으로 만들어짐. 예를들어
 * 	사람들이 자동차를 이용하기 위해서는 우선 공장에서 설계도를 보고 자동차를 만들어야 함.
 * 	객체지향프로그래밍에서도 마찬가지임. 메모리에서 사용하고 싶은 객체가 있다면 우선 설계도로
 * 	해당하는 객체를 만드는 작업이 필요함.
 * 	자바에서 설계도가 바로 클래스(Class)임.
...
public class Ex01_Class {
	...
	// 객체 생성 시점에 해당 데이터타입(자료형)에 맞게 알아서
	// 해당 자료형의 default 값을 할당을 해 줌.
