...
/*
 * 싱글턴(singleton) 방식?
 * - 생성자를 private 접근지정자를 ㅅ ㅏ용하여 외부에서 직접 접근을
 *   못하게 불허하는 방식.
 * - 외부에서 해당 클래스의 인스턴스를 new로 생성하지 못하도록 하는 방식.
 * - 사용하는 이유 : 매번 객체를 생성하개 되면 메모리 성능이 느려지는 
 *              단점이 발생하기 때문.
 */

class Company {

	private static Company instace = null;

	private Company() { // private으로 외부접근 막고
	}

	public static Company getInstance() { // 내부에서 인스턴스 생성.
		if (instace == null) {
			instace = new Company();
		}
		return instace;
